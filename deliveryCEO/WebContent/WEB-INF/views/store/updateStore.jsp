...
<body>
	...
	<div class="content-wrap">
		<form action="/CEO/updateStoreInfo" method="POST"
			...
			<table class="table">
				...
				<tr>
					<th>아이디</th>
					<td>
						<!-- 로그인한 아이디 불러옴 --> <input type="text" name="storeCEO"
						id="storeCEO" class="form-control" value="<%=ceo.getCeoId()%>"
						readonly="readonly">
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><input type="text" name="storeRest" id="storeRest"
						class="form-control" value="<%=store.getStoreRest()%>"></td>
				</tr>
				<tr>
					<th>첨부파일</th>
					<td>
						<%
							if (store.getStoreFilePath() == null) {
						%> <input type="file" name="fileName" onchange="LoadImg(this)">
						<%
							} else {
						%> <input type="file" name="fileName"
						value="/filepath/01_store/<%=store.getStoreFilePath()%>"
 ...
 %>
				...
				<tr>
					...
					<td><textarea rows="3" cols="50" name="storeDetail"
							id="storeDetail" class="form-control" style="resize: none;"><%=store.getStoreDet()%></textarea>
					</td>
				</tr>
				<tr>
					<th colspan="2" style="text-align: center;"><input
						type="submit" class="btn btn-primary btn-lg" value="변경하기"></th>
				</tr>
			</table>
		</form>
	</div>
